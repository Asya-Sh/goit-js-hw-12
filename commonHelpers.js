...
</li>`}function f(){u.innerHTML=""}const H="/goit-js-hw-12/assets/error-03aea49c.svg",I="/goit-js-hw-12/assets/warning-899e47f1.svg",w=document.querySelector(".loader"),h=document.querySelector(".load-more");function L(){w.classList.remove("is-hide")}function i(){w.classList.add("is-hide")}function g(e){m.error({message:e,backgroundColor:"#EF4040",messageColor:"#fff",position:"topRight",iconUrl:H})}function x(){h.classList.remove("is-hide")}function d(){h.classList.add("is-hide")}function C(e){m.warning({message:e,position:"topRight",iconUrl:I})}const T=document.querySelector(".js-form"),v=15;let c=1,l;const U="Sorry, there are no images matching your search query. Please try again!",k="Error, empty field",A="We're sorry, but you've reached the end of search results.";T.addEventListener("submit",D);h.addEventListener("click",N);async function D(e){if(e.preventDefault(),L(),f(),d(),l=e.target.elements.search.value.trim().split(" ").join("+"),!l){f(),g(k),i();return}try{const o=await p(l,c);o.data.hits.length===0?(i(),g(U)):(y(o.data.hits),b(),x()),i(),o.data.totalHits<=v&&d()}catch{console.log(result),i()}e.target.reset()}async function N(){L(),c+=1;try{const e=await p(l,c),s=Math.ceil(e.data.totalHits/v);y(e.data.hits),b(),s===c&&(d(),C(A)),i()}catch(e){console.log(e),i()}}function b(){const s=u.firstElementChild.getBoundingClientRect().height*2;window.scrollBy({top:s,behavior:"smooth"})}
